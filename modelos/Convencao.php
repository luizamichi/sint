<?php
	...
	/**
	 * Classe Convenção com atributos simples (ID, título, arquivo PDF, tipo, usuário, data e status)
	...
	class Convencao {
		 ...
		 * @var string $titulo Descrição da convenção
		 * @var string $arquivo Endereço do arquivo em formato PDF
		 * @var boolean $tipo Vigente (TRUE) ou anterior (FALSE)
		...
		public function __toString() {
			...
			"\nTIPO: " . ($this->tipo ? "VIGENTE" : "ANTERIOR") .
		...
		/**
		 * Atribui um novo valor ao título da convenção
		 * @param string $titulo Descrição sobre a convenção com no mínimo 6 e no máximo 128 caracteres
		 * @return boolean Sucesso ou fracasso na operação
		 */
		...
		public function hashArquivo(string $arquivo) {
			if(strcmp(strtolower(substr($arquivo, -3)), "pdf") == 0 && strlen($arquivo) >= 5) {
				if(strlen($arquivo) > 31) {
					$this->arquivo = mb_strtolower("uploads/convencoes/" . date("YmdHis") . substr($arquivo, -31));
				}
				else {
					$this->arquivo = mb_strtolower("uploads/convencoes/" . date("YmdHis") . $arquivo);
				}
				return TRUE;
			}
			else {
		...
		/**
		 * Atribui um novo valor ao tipo
		 * @param boolean $tipo Booleano referente ao tipo vigente ou anterior
		 * @return void
		 */
		 ...
		 * @param Usuario $usuario Usuário responsável pela convenção
		 * @return boolean Sucesso ou fracasso na operação
		 */
		...
		/**
		 * Atribui um novo valor ao status da convenção
		...
		/**
		 * Retorna o título da convenção
		 * @return string Descrição da convenção
		 */
		...
		/**
		 * Retorna o tipo da convenção
		 * @return boolean Vigente ou anterior
		 */
		...
		/**
		 * Retorna o usuário que criou a convenção
		...
		/**
		 * Retorna a data e hora da criação ou edição da convenção
		...
		/**
		 * Retorna o status de atividade da convenção
		...
		/**
		 * Valida a convenção certificando todos os atributos
		 * @return boolean Sucesso ou fracasso na operação
		 */
		public function valida() {
			if($this->id && $this->titulo && $this->arquivo && $this->tipo && $this->usuario->valida() && $this->data && $this->status) {
				return TRUE;
			}
			else {
				return FALSE;
