...
class Historico():
	...
	Atributos:
		...
		titulo (str): Nome da diretoria e ano de gestão
		documento (Arquivo): Documento
		usuario (Usuario): Usuário
		data (datetime): Data de cadastro do histórico
		ativo (bool): Inativo (False) ou ativo (True)
	'''
	__slots__ = ['_data', '_documento', '_id', '_ativo', '_titulo', '_usuario']

	def __init__(self, id: int=0, titulo: str='', documento: str='', usuario: Usuario=Usuario(), data: datetime=datetime.now(), ativo: bool=False):
		self._id = id
		self._titulo = titulo
