...
class Boletim():
	...
	Atributos:
		...
		data (datetime): Data de cadastro do boletim
		ativo (bool): Inativo (False) ou ativo (True)
	'''
	__slots__ = ['_ativo', '_data', '_documento', '_id', '_imagem', '_titulo', '_usuario']

	def __init__(self, id: int=0, titulo: str='', documento: str='', imagem: str=None, usuario: Usuario=Usuario(), data: datetime=datetime.now(), ativo: bool=False):
		self._id = id
		self._titulo = titulo
