...
class Juridico():
	...
	Atributos:
		...
		titulo (str): Nome da lei, artigo ou referência ao documento
		descricao (str): Descrição resumida
		documento (Arquivo): Documento
		usuario (Usuario): Usuário
		data (datetime): Data de cadastro do jurídico
		ativo (bool): Inativo (False) ou ativo (True)
	'''
	__slots__ = ['_data', '_descricao', '_documento', '_id', '_ativo', '_titulo', '_usuario']

	def __init__(self, id: int=0, titulo: str='', descricao: str=None, documento: str='', usuario: Usuario=Usuario(), data: datetime=datetime.now(), ativo: bool=False):
		self._id = id
		self._titulo = titulo
