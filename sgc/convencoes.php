<?php
	...
	if(!isset($_SESSION["usuarioModelo"])) {
		...
		if(isset($_SESSION["tuplas"])) {
			...
			$tamanho = $convencaoDAO->tamanhoAtivo();
			$paginas = max(ceil($tamanho / $quantidade), 1);
			$tuplas = $convencaoDAO->listarIntervaloAtivo(($p - 1) * $quantidade, $quantidade);
		}
	}
?>
...
<head>
	...
	<meta name="description" content="Página de gerenciamento de convenções">
...
<body>
	...
	<div class="container-fluid">
		<h1 class="mt-3 col-sm">Convenções</h1>
		<div class="col-sm">
			<form action="../controladores/procurar_convencao.php" method="POST" id="procurar_convencao">
				<div class="form-row align-items-center">
					...
					<div class="col-auto my-3">
						<a class="btn btn-outline-success ml-1" href="inserir_convencao.php">Adicionar</a>
					</div>
				</div>
			</form>
					...
					<?php
						foreach($tuplas as $tupla) {
							if($tupla != NULL) {
								...
								echo "<td><a href=\"../" . $tupla->getArquivo() . "\" title=\"" . substr($tupla->getArquivo(), 19) . "\" target=\"_blank\"><img class=\"cinza\" src=\"../imagens/pdf.svg\" alt=\"Documento PDF\" width=\"25\" height=\"25\"></a></td>";
								echo "<td>" . ($tupla->isTipo() ? "Vigente" : "Anterior") . "</td>";
								echo "<td>" . $tupla->getData()->format("d/m/Y - H:i:s") . "</td>";
								echo "<td>";
								echo "<a class=\"ml-3\" href=\"alterar_convencao.php?id=" . $tupla->getId() . "\" title=\"Alterar\"><img src=\"../imagens/alterar.svg\" width=\"20\" class=\"hover-azul\" alt=\"Alterar\"></a>";
								echo "<a class=\"ml-3 remover\" id=\"" . $tupla->getId() . "\" data-toggle=\"modal\" data-target=\"#modal\" onclick=\"removerConvencao(" . $tupla->getId() . ")\" title=\"Remover\"><img src=\"../imagens/remover.svg\" width=\"20\" class=\"hover-vermelho\" alt=\"Remover\"></a>";
								echo "</td>";
								echo "</tr>";
							}
						}
		...
		<div class="modal-dialog modal-dialog-centered" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					Tem certeza que deseja remover a convenção selecionada?
				</div>
		...
		$(document).ready(function() {
			$("#inputSelecao").on("change", function() {
				if(this.value == "id") {
					...
					$("#input").replaceWith("<input class=\"form-control\" type=\"date\" id=\"input\" name=\"input\" required>");
				}
			});
		});

		function removerConvencao(id) {
			document.getElementById("botaoRemover").setAttribute("href", "../controladores/remover_convencao.php?id=" + id);
		}
	</script>
</body>
