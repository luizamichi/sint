<?php
	...
	if(!isset($_SESSION["usuarioModelo"])) {
		...
		if(isset($_SESSION["tuplas"])) {
			...
			$tamanho = $bannerDAO->tamanhoAtivo();
			$paginas = max(ceil($tamanho / $quantidade), 1);
			$tuplas = $bannerDAO->listarIntervaloAtivo(($p - 1) * $quantidade, $quantidade);
		}
	}
?>
...
<head>
	...
	<meta name="description" content="Página de gerenciamento de banners">
...
<body>
	...
	<div class="container-fluid">
		<h1 class="mt-3 col-sm">Banners</h1>
		<div class="col-sm">
			<form action="../controladores/procurar_banner.php" method="POST" id="procurar_banner">
				<div class="form-row align-items-center">
					...
					<div class="col-auto my-3">
						<a class="btn btn-outline-success ml-1" href="inserir_banner.php">Adicionar</a>
					</div>
				</div>
			</form>
					...
					<?php
						foreach($tuplas as $tupla) {
							if($tupla != NULL) {
								...
								echo "<td><a href=\"../" . $tupla->getImagem() . "\" title=\"" . substr($tupla->getImagem(), 16) . "\" target=\"_blank\"><img class=\"cinza\" src=\"../imagens/" . (strcmp(strtolower(substr($tupla->getImagem(), -3)), "png") == 0 ? "png.svg" : "jpg.svg") . "\" alt=\"Imagem " . (strcmp(strtolower(substr($tupla->getImagem(), -3)), "png") == 0 ? "PNG" : "JPG") . "\" width=\"25\" height=\"25\"></a></td>";
								echo "<td>" . $tupla->getData()->format("d/m/Y - H:i:s") . "</td>";
								echo "<td>";
								echo "<a class=\"ml-3\" href=\"alterar_banner.php?id=" . $tupla->getId() . "\" title=\"Alterar\"><img src=\"../imagens/alterar.svg\" width=\"20\" class=\"hover-azul\" alt=\"Alterar\"></a>";
								echo "<a class=\"ml-3 remover\" id=\"" . $tupla->getId() . "\" data-toggle=\"modal\" data-target=\"#modal\" onclick=\"removerBanner(" . $tupla->getId() . ")\" title=\"Remover\"><img src=\"../imagens/remover.svg\" width=\"20\" class=\"hover-vermelho\" alt=\"Remover\"></a>";
								echo "</td>";
								echo "</tr>";
							}
						}
	...
	<script>
		...
		function removerBanner(id) {
			document.getElementById("botaoRemover").setAttribute("href", "../controladores/remover_banner.php?id=" + id);
		}
	</script>
</body>
